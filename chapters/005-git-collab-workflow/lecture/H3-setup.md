...
npx husky init        # .husky/ 디렉토리 생성 + package.json prepare 스크립트 추가

# pre-commit hook (commit 직전 실행)
echo "npx lint-staged" > .husky/pre-commit

# commit-msg hook (commit 메시지 작성 직후 실행)
...
추신 1. organization은 자경단의 회사 명함. 5명 되는 순간 옮겨요. 1주일 미루면 1년 미루게 돼요. **명함이 협업의 첫 도구**.

추신 2. team 5개의 페르소나 분리가 권한 5단계의 한 줄 표. 본인이 자경단 맡으면 maintainers 1명으로 1년. 본인 1명이 자경단의 단단함이에요.

추신 3. branch protection 7체크 한 표를 종이에 적어 두세요. 면접 단골 질문. "main 보호 어떻게 하셨어요?" 7개 다 말하면 시니어 신호.

추신 4. CODEOWNERS는 파일 경로의 사람 묶기. 노랭이가 백엔드 건드리면 까미 자동. 한 줄이 합의 30초. **경로가 동료를 불러요**.

추신 5. Conventional Commits 7 prefix를 외우세요. feat·fix·docs·style·refactor·test·chore. + perf·ci·build 3개. 10개로 본인의 모든 commit이 분류돼요. **분류가 협업의 첫 학문**.

추신 6. husky pre-commit 5초의 답답함이 5년의 일관성. 처음 1주일만 답답해요. 그 다음은 손가락 자동. 답답함은 자산.

추신 7. lint-staged는 staged 파일만. 전체 lint는 CI에서. **빠른 hook + 깊은 CI**의 분업.

추신 8. SSH 키는 본인 (사람), deploy key는 prod 서버 (기계). 사람과 기계의 비밀을 분리하면 누출 사고가 1/100. **한 사람의 한 비밀, 한 기계의 한 비밀**.

추신 9. PAT 만료 90일이 표준. 1년은 너무 길어요. 짧은 만료가 사고 비용을 줄여요. **만료가 보안의 무기**.

추신 10. fine-grained PAT은 2022년 새 도구. classic보다 좁아요. 6개월 후 한 개씩 마이그레이션. **새 도구가 사고 비용을 1/10로**.

추신 11. branch protection의 "Include administrators" 한 칸이 본인을 묶어요. 자물쇠 위에 올라가지 않는 자물쇠가 진짜 자물쇠. **자기를 묶는 한 칸이 단단함**.

추신 12. CODEOWNERS 파일 자체의 owner를 maintainers로. 안 하면 누구나 CODEOWNERS 한 줄 고쳐서 자기 권한 확장 가능. **파일의 파수꾼**.

추신 13. 셋업 30분 × 1년 사고 방지 × 5년 = 125시간 절약. 30분 투자 ROI 무한대. **셋업이 자경단의 가장 큰 무기**.

추신 14. 다음 H4는 카탈로그 — 협업 명령어·도구 23~30개의 한 표 + 위험도 신호등 + 매일·주간·월간 리듬. 본 H의 셋업이 H4의 명령어로 손에 잡혀요. 🐾

추신 15. 본 H를 다 읽은 본인이 한 가지 실험 — 본인이 좋아하는 오픈소스 저장소(예: facebook/react, fastapi/fastapi, vercel/next.js)의 `.github/CODEOWNERS`를 열어 보세요. 안 보이면 `CODEOWNERS` 루트도. 큰 회사가 5명 자경단보다 어떻게 더 정밀하게 파일 경로를 사람에게 매핑하는지 1분에 보여요. **큰 회사의 CODEOWNERS 한 페이지가 본인의 5년 학습**. 🐾🐾
