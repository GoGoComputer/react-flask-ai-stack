...
추신 1. 자동화는 5명에도 효과적. branch 자동 삭제·dependabot·release-please 5분 셋업이 매주 25분 절약. **5분 ROI 100배**.

추신 2. dependabot의 weekly + grouping이 자경단 표준. 한 주 5~10 PR이면 본인이 차례대로 머지. 보안 노출 1주일 → 0일.

추신 3. release-please는 Google 도구. semantic-release(npm 표준)도 있어요. 둘 중 본인 stack에 맞는 거. 자경단은 release-please.

추신 4. CHANGELOG.md는 사용자의 일기장. 매 release마다 한 줄씩이 자경단의 나이테. 1년 후 50줄. 5년 후 250줄. 한 줄이 한 release.

추신 5. stale bot은 자경단의 청소부. 60일 + 90일의 알람 + 닫기. 죽은 issue가 살아 있는 것 같이 보이면 본인의 우선순위가 깨져요.

추신 6. hot file 분석은 매월 한 번. 상위 5개 파일이 conflict 80%. 쪼개기가 가장 빠른 처방.

추신 7. monorepo는 자경단 5명에도 OK. `npm workspaces` 또는 `pnpm workspaces`. 빌드 5분+ 되면 Nx 도입.

추신 8. 5단계 진화표(5·10·25·50·50+)가 자경단의 미래. 매년 어디 단계인지 점검. 단계가 바뀌면 도구도 바뀜.

추신 9. workflow 회고 5단계(숫자·사람·도구·결심·한페이지)가 진화 의식. 1시간이 1년의 진화.

추신 10. signed commit ON은 6개월 후 검토. 셋업 비용 5명 × 1시간. 큰 회사로 갈 때 필수.

추신 11. on-call rotation은 1년 후 + 사용자 1만 명+. 5명이 1주씩. 새벽 알람을 1년에 한 번.

추신 12. 자동화 ROI 표 — 셋업 5분 × 절약 25분/주 × 50주 = 1,250분/년 = 20시간. 5명 × 4도구 = 80시간/년. **80시간이 한 도구 묶음의 가치**.

추신 13. branch 자동 삭제는 origin만. local은 본인이 `git branch -d` 또는 `fetch.prune=true`로. 두 곳 다 청소가 단단.

추신 14. release-please의 `release-type`은 stack에 맞게 — node·python·go·rust·simple·java. 자경단 react+fastapi면 `node` (frontend) + `python` (backend) 둘 셋업.

추신 15. 다음 H7은 원리/내부 — branch protection이 GitHub 내부에서 어떻게 강제되는가, CODEOWNERS가 어떤 알고리즘으로 매칭되는가, conflict 알고리즘 깊이. 본 H의 운영이 H7의 원리로 깊어져요. 🐾

추신 16. 자경단 1년·3년·5년 곡선의 핵심은 **사용자 곡선**. 협업이 잘 운영되면 사용자가 100배 성장. 협업이 운영의 그릇.

추신 17. 매년 12월 마지막 주 1시간 회고가 평생 의식. 5년 후엔 본인이 회고 방법을 동료에게 가르침. **회고가 자산**.

추신 18. 본 H의 자동화 도구 5종(branch 삭제·dependabot·stale·release-please·Pages)은 모두 무료(public repo) 또는 Team plan 포함. 자경단 5명 비용 0~$20/월.

추신 19. 5명 자경단의 1년 후 회고에서 가장 자주 나오는 문장 — "1년 전에 자동화 안 했으면 지금 폭발했을 것". 매주 1,250분의 절약이 1년 후 자경단의 산소.

추신 20. 본 H를 다 읽은 본인이 한 가지 결심 — 본인의 첫 자경단(또는 다음 프로젝트)에서 4개 자동화(branch 삭제·dependabot·stale·release-please)를 첫 주에 셋업. 30분이 1년의 운영을 사요.

추신 21. 5사고 일지(9절)를 모니터 옆에 붙여 두세요. 1년 동안 5사고 중 3개 즈음 마주칠 거. 처방을 미리 알면 5분, 모르면 5시간.

추신 22. dependabot grouping은 자경단의 친구. `react`·`@types/react`·`react-dom` 한 묶음 PR. 50개 → 15개. 본인이 한 묶음을 한 번에 검수.

추신 23. release-please가 자동 생성한 PR을 머지 직전 본인이 5분 검수. CHANGELOG·release 노트 자동이지만 한 줄 손 검수가 5명 사용자의 마이그레이션 부담 결정.

추신 24. `BREAKING CHANGE` 한 줄은 사용자 5명의 하루. 까미·노랭이가 PR 본문에 쓸 때 본인이 한 번 더 검토. 진짜 break면 v2.0.0 + 마이그레이션 가이드, 작으면 빼기.

추신 25. hot file 분석은 매월 첫 주 월요일. 한 줄 명령어 `git log --since='1 month ago' --pretty=format: --name-only | sort | uniq -c | sort -rn | head`. 30초 진단.

추신 26. App.tsx 1000줄 → 5파일 200줄. CODEOWNERS에 owner 분리. conflict 50번 → 8번. 1/6 감소가 자경단 한 달의 산소.

추신 27. monorepo는 자경단 5명의 친구. polyrepo는 50명+에서 검토. 도구가 인원에 맞게 진화.

추신 28. monorepo + Nx (또는 Turborepo)는 빌드 캐시. 변경된 패키지만 빌드 → CI 5분 → 30초. 큰 자경단의 산소.

추신 29. signed commit ON 전에 5명 동시 셋업 1주일. SSH signing(2022년)이 GPG보다 5분 빠름. 기존 SSH 키 재사용.

추신 30. on-call rotation은 1년 후 + 사용자 1만 명+. 5명이 1주씩 = 5주 사이클. 새벽 알람 1년에 1번/사람.

추신 31. workflow 매년 회고는 12월 마지막 금요일 14:00 1시간. 자경단 5명이 모여 5단계. 본인이 회고록 1페이지로 정리.

추신 32. 회고 5단계의 4번 — 다음 해 결심. "5명 → 7명 시점 staging 추가", "1년 후 signed commit ON" 식. 결심을 `docs/ROADMAP.md`에 박기.

추신 33. 자경단 1년 곡선의 핵심 — 협업이 운영의 그릇. 협업 잘 되면 사용자 100배 성장. 못 되면 사용자 0.

추신 34. 큰 오픈소스(react·fastapi·next.js)의 `.github/` 폴더 구경하세요. 자경단의 5년 후가 거기 있어요. 본 H의 도구 80%가 거기 있어요.

추신 35. 자동화 ROI 표 — 셋업 30분 × 절약 80시간/년 × 5년 = 400시간. **400시간이 자경단의 진짜 자산**.

추신 36. 자경단 표준 자동화 5종 — branch 삭제·dependabot·stale·release-please·docs Pages. 셋업 30분, 매주 25분 절약.

추신 37. release-please의 `release-type`별 — node·python·go·rust·java·simple. 자경단 stack에 맞게. monorepo면 두 type 동시 셋업.

추신 38. CHANGELOG.md는 자경단의 일기장. 매 release마다 한 줄. 5년 후 250줄. 한 줄이 한 사용자 가치.

추신 39. branch protection의 7체크 + CODEOWNERS + auto-merge 셋이 자경단의 평생 자물쇠. 30분 셋업이 5년 안전.

추신 40. 본 H의 마지막 회수 — 자경단의 1년·3년·5년이 셋업 30분의 곱셈. 첫 주의 30분이 5년의 산소.

추신 41. workflow 회고에서 "패턴을 바꿔야 할까?"가 가장 큰 결정. 5명 → 25명 시점에 GitHub Flow → Trunk-based 검토. 인원과 인프라가 함께 결정.

추신 42. 자경단 운영의 황금 규칙 5개 — 매주 점검(자동화도)·매월 통계(hot file)·매분기 도구 점검(branch protection)·매반기 마이그레이션(PAT·signing)·매년 회고(1시간 5단계).

추신 43. 본 H의 모든 사례·도구·시뮬레이션은 강사가 자경단 운영 5년 시뮬을 머리에 그리며 쓴 것. 본인이 진짜 5년 운영하면 본 H의 5사고 외에 5사고 더 만남. 자기만의 사고 일지가 자기만의 평생 처방.

추신 44. 다음 H7은 원리/내부 — branch protection이 GitHub 내부에서 어떻게 강제되는가, CODEOWNERS 매칭 알고리즘, conflict 알고리즘 깊이, 분산 git의 운영 비밀. 본 H의 운영이 H7의 원리로 깊어져요. 🐾

추신 45. 본 H를 두 번 읽으세요. 첫 번째는 자동화·통계·진화·회고의 큰 그림. 두 번째는 5사고 일지. 두 번 읽기 + 본인의 첫 자경단 셋업이 1년 후 진짜 운영 직관.

추신 46. 자경단 5명이 50명·500명이 되어도 본 H의 의식 5개(매주·매월·매분기·매반기·매년)는 그대로. 인원이 바뀌어도 의식은 평생. **의식이 협업의 시계**.

추신 47. dependabot의 `open-pull-requests-limit`을 5로 설정. 한 번에 5 PR만. 본인이 한 주에 5개 차례대로 머지. 부담 1/5.

추신 48. release-please는 `release-as` 옵션으로 다음 release 강제 가능. 자경단의 첫 v1.0.0 결정 시 `release-as: 1.0.0`을 PR 본문에. **수동과 자동의 균형**.

추신 49. monorepo의 PR이 frontend·backend 양쪽 변경하면 CODEOWNERS가 두 팀 모두 자동 reviewer. 까미·노랭이 둘 다 알람. 5분 페어 자연 발생.

추신 50. 5사고 표(9-6절)를 자경단의 README 또는 CONTRIBUTING.md에 박아 두세요. 새 멤버가 합류 시 1주일 안에 5사고를 알게 됨. **사전 학습이 사고 비용 1/10**.

추신 51. workflow 회고의 결심 한 줄을 commit 메시지로 — `docs(workflow): 2027 결심 — staging 환경 도입` 식. 결심이 git history에 박혀 1년 후 자기 자신에게 읽힘.

추신 52. signed commit ON은 큰 회사로 갈 신호. 6개월 셋업 + 1년 안정 + 1년 후 ON. 자연스러운 진화.

추신 53. 자동화의 지표는 **개입 비율**. 본인이 매주 자동화에 손대는 시간 vs 자동화가 운영 시간. 1:10 이상이면 셋업 추가. 1:50이 표준.

추신 54. 본 H의 5사고 중 본인이 가장 무서워하는 사고를 골라 처방을 README에 박기. 무서움이 사전 학습의 동기.

추신 55. 자경단 5년 운영 시뮬(5절)의 사용자 100배 성장은 **운영의 곱셈 효과**. 협업 잘 되는 자경단은 연 2배 성장, 5년이면 32배. 운영이 그 곱셈의 그릇.

추신 56. 본 H를 다 읽은 본인이 한 가지 행동 — 본인의 자경단(또는 다음 프로젝트)에 dependabot.yml 한 파일 추가 PR. 5분 작업이 1년의 첫 자동화.

추신 57. release-please의 PR을 머지하지 않고 1주일 동안 모았다가 한 번에 release하는 패턴도 가능. 자경단 매주 금요일 머지가 표준이지만 변형 가능.

추신 58. 운영의 세 척도 — **속도**(release/주)·**안정성**(incident/월)·**행복**(개발자 만족도). 셋이 균형. 한쪽으로 치우치면 다른 둘이 무너짐. 자경단의 황금 비율 — 속도 1·안정 1·행복 1.

추신 59. 자경단의 운영 1년 후엔 본 H를 다시 읽으세요. 같은 H가 다른 깊이로 읽힘. 1년의 경험이 본 H의 30%를 새로 보여 줘요. **읽기는 평생**.

추신 60. 본 H의 마지막 한 줄 — 운영은 셋업 30분의 곱셈, 매주 점검의 곱셈, 매년 회고의 곱셈. **곱셈이 자경단의 산소**.

추신 61. 자경단 운영의 마지막 비밀 — **잘 운영되는 협업은 보이지 않아요**. 매일 30분 PR 머지·매주 자동화 점검·매년 회고가 자연스럽게 흐르면, 5명이 다른 일에 집중. 보이지 않는 운영이 진짜 운영.

추신 62. 본 H를 끝낸 본인이 한 결심 — 본인의 첫 자경단 organization 셋업 후 일주일 안에 본 H의 자동화 5종 + 의식 5개를 박기. 일주일이 5년의 산소.

추신 63. 5사고 일지의 마지막 회수 — 사고는 자경단의 진짜 학습이에요. 사고 5개 중 3~4개를 마주치고 처방하면, 본인의 운영 직관이 5년의 시니어 신호. **사고가 시니어의 학교**.

추신 64. 본 H 마무리 — 운영은 손가락이 아닌 의식, 의식이 자동화의 곱셈이에요. 30분 셋업·1주 점검·1년 회고가 자경단의 호흡이에요. 본 H의 첫 도구를 오늘 박으세요. 🐾🐾🐾
