...
17. 흔한 실수 다섯 가지 + 안심 멘트 — 도구 카탈로그 학습 편
18. 다음 시간 예고 + 한 줄 정리
...
§16에서 함정과 FAQ를 봤어요. 이번엔 학습 자세 함정 다섯을 짚고 가요.

첫 번째 함정, 도구 14개 다 외우려고. 본인이 ping·traceroute·dig·curl·nc·openssl·ss·lsof·tcpdump·tshark·iperf3·arp·route 다 머리에 박으려고. 안심하세요. **매일 치는 건 ping·dig·curl 셋.** 나머지 11개는 평생 검색이에요. 첫 챕터 약속 — 외우지 말고 자주 만나기.

두 번째 함정, tcpdump를 production에서 그냥 돈다. 본인이 디버깅하려고 운영 서버에 tcpdump. 안심하세요. **패킷 다 캡처해서 디스크 먹어요.** 1Gbps면 1초에 125MB, 5분이면 37GB. 디스크 풀 사고. 항상 (1) `-c 1000` 패킷 수 제한, (2) `-w file.pcap` 파일 저장, (3) staging 먼저. production은 좁게 — `tcpdump -c 100 host A.B.C.D and port 80`.

세 번째 함정, ping의 RTT를 실제 지연으로. 본인이 ping 50ms 보고 "내 네트워크 50ms 지연" 단정. 안심하세요. **ping은 ICMP, 실제 트래픽은 TCP/HTTP.** 다른 큐 거쳐요. 정확한 측정은 `curl -w "@curl-format.txt"` 로 HTTP latency 분해. SLA 측정의 기본.

네 번째 함정, openssl 명령을 한 번에 다 익히려고. s_client·verify·x509·req·rsa·dgst·enc·s_server. 안심하세요. **세 개만.** s_client (확인), x509 (정보), verify (검증). 나머지는 평생 검색.

다섯 번째 함정, 가장 큰 함정. **도구 출력을 다 이해하려고.** 본인이 tshark 한 패킷 출력 50줄 다 풀려고. 안심하세요. **첫날엔 헤더 한 줄, 출발지/목적지/포트만.** 깊이는 두 해 동안 매번 만나며 자라요. 한 줄씩 친근해지면 충분.
...
> - §17 첫째 함정 14개 도구 중 매일 3개(ping·dig·curl)는 80/20 법칙. 나머지 11개는 사고 시점에만. 도구는 검색하면 30초, 외우면 한 달이면 잊음.
> - §17 둘째 함정 tcpdump production 위험 — Netflix는 tcpdump-as-a-service 내부 도구로 5분 자동 종료, 압축, S3 업로드. 운영 환경 안전 패턴의 참고. 본인 회사도 emergency.sh에 tcpdump-safe 함수 한 줄 권장.
> - §17 셋째 함정 ping vs HTTP latency — ping(ICMP)는 커널 레벨 처리, HTTP는 user space 거쳐 처리. 큐 위치가 달라 RTT 차이 가능. SLA 측정은 항상 실제 트래픽으로.
> - §17 넷째 함정 openssl 명령 100+개 — `openssl list -commands` 로 전체 목록. 매일 3개로 95% 커버. PKI 깊이는 Ch 069 보안에서.
> - §17 다섯째 함정 도구 출력 깊이 학습 — Spaced Repetition + Active Recall 학습 이론. 첫 만남 30%, 5번째 만남 90%. 뇌의 학습 메커니즘이 그래요.
> - 분량: 이 H4는 약 19,000자(공백 제외). 음독 60분. 디벨롭 이력: 2026-04-30 §17 "흔한 실수 도구 카탈로그 학습 편" 신설 / 개발자 노트 6→11항목 보강.
