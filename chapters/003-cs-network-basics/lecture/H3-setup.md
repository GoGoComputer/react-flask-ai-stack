...
14. 흔한 함정 다섯 개
15. FAQ 다섯 개
16. 회차 지도 — 환경 점검이 다시 찾아올 시간
17. 흔한 실수 다섯 가지 + 안심 멘트 — 환경 점검 학습 편
18. 다음 시간 예고 + 한 줄 정리
...
> ▶ **같이 쳐보기** — 인터넷에서 보이는 본인 빌딩 주소 (공인 IP)
>
> ```bash
...
> ▶ **같이 쳐보기** — 우리 동네 우체통(게이트웨이)까지 왕복 시간
>
> ```bash
> # macOS
...
DNS 조회의 가장 첫 단계가 어디라고 했죠. 본인 노트북의 stub resolver. 그 stub resolver가 DNS 서버를 부르기 전에 먼저 보는 파일이 있어요. /etc/hosts. macOS와 Linux 둘 다 같은 파일이에요. 한 번 펴 보세요.

> ▶ **같이 쳐보기** — DNS 보다 먼저 보는 작은 책 한 번 펴기
>
> ```bash
> cat /etc/hosts
> # 127.0.0.1       localhost
...
오늘 본인 노트북의 네트워크 환경을 캐는 명령을 많이 만났어요. 자주 빠지는 학습 함정 다섯을 짚고 가요.

첫 번째 함정, 환경 점검 명령을 다 외우려고 한다. ifconfig·ip addr·route·netstat·ss·dig·resolvectl. 본인이 머리에 박으려고. 안심하세요. **macOS 5개 + Linux 5개 = 10개.** 매일 치는 건 ifconfig(또는 ip addr)·dig·curl 셋. 나머지는 평생 검색. netinfo alias 한 줄로 묶어서 손에 익히세요.

두 번째 함정, /etc/hosts 수정으로 모든 DNS 문제 해결. 본인이 DNS 안 풀리면 /etc/hosts에 직접 추가. 안심하세요. **임시 디버깅용은 OK, 영구 해결책은 아닙니다.** /etc/hosts는 본인 노트북 한 대만. 운영 서버는 진짜 DNS 설정. 두 해 후 본인이 회사에서 /etc/hosts에 prod 도메인 박아두면 같은 팀 다른 사람이 본인 코드 못 돌려요.

세 번째 함정, 사설 IP와 공인 IP 헷갈리기. 본인이 `ifconfig`로 본 192.168.x.x를 인터넷 주소로 생각. 안심하세요. **`curl ifconfig.me` 로 진짜 공인 IP 확인.** 사설 IP는 본인 집 안에서만 의미. 공인 IP는 지구적으로 유일. 두 IP를 헷갈리면 방화벽 설정이 통째로 어긋남. Ch001 H3과 Ch003 H1에서 두 번 본 함정. 세 번째 만남에서 손에 박혀야 해요.

네 번째 함정, MTU 1500 무조건 적용. 본인이 MTU 값을 환경 무관하게 1500으로. 안심하세요, 함정 풀이부터. **VPN은 보통 1400 미만, PPPoE는 1492.** 환경마다 달라요. 잘못 잡으면 큰 패킷이 단편화되어 느려요. `ping -M do -s 1472` (Linux) 또는 `ping -D -s 1472` (macOS)로 측정. 첫 챕터에서는 "MTU는 환경마다 다르다"는 한 줄만.

다섯 번째 함정, 가장 큰 함정. **회사 네트워크와 집 네트워크 차이 모르고 한 환경에서만 익힘.** 본인이 회사에서만 익히면 집에서 안 통해요. 안심하세요. **두 환경 다 쳐 보세요.** 회사: ICMP 차단·DNS 강제·HTTP 프록시. 집: NAT·DDNS·UPnP. 두 환경 차이를 손에 익혀야 두 해 후 어디 가도 진단 가능. Ch003 8시간 동안 회사 한 번, 집 한 번씩 같은 명령. 그게 진짜 환경 학습.
...
> - §17 첫째 함정 명령 10개 분리 — macOS BSD 도구(ifconfig, netstat, route)와 Linux iproute2 도구(ip addr, ss, ip route). iproute2가 더 표준이라 macOS도 점점 그쪽으로 추세. Apple도 ifconfig를 deprecate 중.
> - §17 둘째 함정 /etc/hosts vs DNS 순서는 /etc/nsswitch.conf의 hosts 줄로 결정. macOS는 systemd 없어 /etc/resolver/ 디렉토리로 도메인별 분기.
> - §17 셋째 함정 사설 IP 3블록(RFC 1918) 매핑 — 본인 가정 공유기 192.168.x, 회사 VPN 10.x, AWS VPC 기본 10.0.0.0/16. 세 환경의 진짜 IP 차이.
> - §17 넷째 함정 MTU 측정 — Linux `ping -M do -s 1472` 가 GNU iputils, macOS `ping -D -s 1472` 가 BSD 표준. -M/-D 옵션 차이 때문에 자동화 시 OS 분기 필요.
> - §17 다섯째 함정 환경 차이 학습 — 회사 ICMP 차단 (보안), DNS 강제 (사내 서비스), HTTP 프록시 (감사), TLS 인터셉트 (DLP). 네 가지 정책의 의도와 우회법은 Ch 069 보안 시간에 정식 학습.
...
19. 두 해 후 본인이 회사 신입 첫날 받는 진짜 흔한 질문 — "본인 노트북 IP가 뭐예요?" 본인 답: "사설은 192.168.0.X, 공인은 회사 VPN 통하면 10.X.X.X로 보여요." 두 IP의 분리가 신입의 진짜 신호. 외운 답 아니라 자기 환경을 본 답이라는 게 면접관에게 보여요. 그 30초가 본인 합격을 결정.
20. 마지막의 마지막. 본인이 두 해 후 새벽 3시 운영 사고 만나면 첫 한 줄이 정확히 오늘 만든 netinfo. 그 한 줄이 본인의 진짜 응급실 첫 카드. 5초 만에 환경 한 화면. 진단의 첫 30초. 그게 오늘 한 시간의 진짜 결과물입니다. 본인이 만든 작은 도구가 새벽 3시의 본인을 살리는 진짜 친구. 작은 한 줄, 큰 의미. 두 해 후 본인의 도구 가족이 한 명 더 늘었다는 작은 자축. 진짜로요.
