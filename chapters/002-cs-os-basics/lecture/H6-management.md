...
20. 흔한 실수 다섯 가지 + 안심 멘트 — 응급처치 학습 편
21. 마무리 — H7 kernel vs user space 로
👨‍💻 개발자 노트
...
본인이 오늘 응급처치 도구를 손에 익혔어요. 자주 빠지는 학습 함정 다섯을 짚고 가요.

첫 번째 함정, 처방을 진단보다 먼저 한다. 알람 울리면 본인이 무서워서 바로 명령. 안심하세요. **진단 먼저, 처방 나중.** Ch001 H6의 첫 약속. 30초만 멈추세요. ps·top·df로 어떤 사고인지 보고 그 다음에 처방. 30초 멈춤이 30분의 사고 회복 시간을 살려요.

두 번째 함정, kill -9를 첫 카드로. 본인이 응답 없는 프로세스에 바로 -9. 안심하세요. **TERM(15) 먼저, 5초 기다린 후 -9.** -9는 데이터 정리 시간 안 줘요. DB 프로세스를 -9로 끄면 데이터 손상. 5초의 차이가 24시간을 살려요.

세 번째 함정, 디스크 풀에 `rm -rf /tmp/*` 친다. 본인이 임시 파일 정리하려고. 그런데 다른 앱이 /tmp의 본인 파일을 쓰고 있을 수 있어요. 안심하세요. **`du -sh /tmp/*` 로 큰 폴더 먼저 확인.** 또는 `find /tmp -mtime +7 -delete` 로 7일 이상 안 쓴 것만. 한 단계 더 가는 게 사고 막아요.

네 번째 함정, OOM이면 RAM 늘리기만 생각. 본인이 OOM 알림 받자마자 인스턴스 업그레이드 결정. 비싸요. 안심하세요. **메모리 누수가 진짜 원인일 수 있어요.** 사용량이 시간에 따라 증가하면 누수. 코드 버그 한 줄이 RAM 8GB를 64GB로 늘리는 사고를 막아 줘요. RAM 늘리기는 마지막 카드. procmon.sh 메모리 그래프가 30만 원을 살려요.

다섯 번째 함정, 가장 큰 함정. **사고 풀고 회고 안 한다.** 새벽 3시 사고 풀고 너무 피곤해서 바로 자요. 안심하세요, 진실 풀이부터. **사고 자체는 자산이 아니에요. 회고가 자산을 만들어요.** Ch001 H6의 3줄 회고. (1) 어떤 사고였나, (2) 무엇으로 풀었나, (3) 다음에 미리 막으려면. 1분이에요. 1분이 6개월 후 같은 사고를 막아요. 회고 안 한 사람은 같은 사고를 또 풀고, 회고한 사람은 다음 사고가 안 나요. 두 해 후 본인 면접에서 "어려운 사고 풀어 본 경험" 질문에 그 회고 노트가 본인의 진짜 답.

다섯 함정 미리 알아둔 본인이 두 해 동안 한 박자 빠르게 손이 움직여요.

여섯 번째 함정 보너스. **혼자 새벽 사고를 다 풀려고 한다.** 본인이 새벽 3시 알람에 일어나서 30분 동안 혼자 끙끙대요. 안 풀려요. 좌절감. 안심하세요. **5분 시도 후 안 풀리면 동료에게 콜.** 시니어 운영팀의 표준 페어 운영(pair on-call) 규칙. 한 명이 혼자 새벽 3시에 풀려고 하면 5분 만에 풀릴 일도 30분으로 늘어요. 잠 안 깬 머리는 진단 능력 50%. 두 명이 머리 맞대면 100%. 두 해 후 본인 회사의 on-call 채널 규칙 첫 줄. "혼자 풀지 마세요." 동료의 5분이 본인의 30분을 살려요.

일곱 번째 함정 보너스. **로그를 안 본다.** 본인이 사고 났을 때 명령만 치고 로그 파일을 안 봐요. 안심하세요. **/var/log 가 진단의 진짜 첫 자리.** macOS의 `log show --last 1h` 또는 Linux의 `journalctl -xe`. 사고 직전 1시간 로그가 거의 항상 단서를 줘요. ps·top은 현재 상태, 로그는 시간 흐름. 두 가지를 같이 봐야 진짜 진단. 로그 안 보고 ps만 보는 사람은 사고를 절반만 푸세요. 두 해 후 모든 production 시스템에서 로그 = 첫 도구.

여덟 번째 함정 보너스. **동일 사고가 반복되는데 근본 처방을 안 한다.** 본인이 매주 같은 디스크 풀 사고를 풀어요. 매번 `rm -rf` 한 번. 그런데 다음 주 또 일어나요. 안심하세요. **3번 같은 사고 = 자동화 신호.** 좀 더 깊이 보세요. 어떤 로그가 너무 빨리 쌓이고 있나? logrotate 설정이 빠졌나? 디스크 한도 알람이 미리 안 와서 사고 직후에야 알았나? 매주 사고 30분 × 50주 = 25시간/년. 25시간이면 자동화 한 번 만들 시간. 한 번의 자동화가 1년의 새벽을 살려요. SRE의 진짜 사고 방식 — **"같은 사고를 두 번 풀면 시스템을 고치고, 세 번 풀면 본인이 잘못한 거다."**

여덟 함정 다 들으셨어요. 외우진 마세요. 그저 본인이 빠졌을 때 한 번씩 떠오르시면 돼요. 진단 우선·-9 신중·du 먼저·메모리 누수·회고·페어·로그·자동화. 여덟 함정을 손에 둔 본인이 두 해 후 시스템 의사로 자라요. 미리 알면 미리 풀어요.
...
> - 좀비(Z) 프로세스의 정확한 처리 — 부모가 wait()/waitpid() 호출하면 자동 정리. SIGCHLD 핸들러 등록도 한 가지 패턴. Linux의 prctl(PR_SET_CHILD_SUBREAPER)로 도커 같은 컨테이너 init이 좀비를 거두는 메커니즘. Ch 047 Docker tini/dumb-init 시간에 회수.
> - OOM Killer는 Linux 커널의 oom_score 알고리즘으로 점수 매겨 가장 높은 프로세스 종료. macOS는 jetsam (모바일 스타일). `dmesg | grep -i kill` 또는 `log show --predicate 'subsystem == "com.apple.kernel.jetsam"'` 로 확인. Ch 091 성능에서 깊이.
> - df vs du의 차이 — df는 파일시스템 메타데이터, du는 실제 디렉토리 트리 순회. df가 du와 다른 이유: deleted but open files (lsof로 확인), reserved blocks (5%, root 전용), file system overhead. 두 해 후 운영자 단골 질문.
> - traceroute는 ICMP/UDP TTL 트릭. 일부 회사 네트워크에서 ICMP 차단 시 `traceroute -T` (TCP) 또는 `tcptraceroute` 사용. macOS는 BSD traceroute, Linux는 GNU traceroute.
> - renice는 nice 값 -20(최우선)~+19(최하위) 범위. `renice +19 PID` 로 백그라운드 작업 우선순위 낮추기. macOS는 -20~+20, Linux는 동일. Ch 087 알고리즘의 OS 스케줄러 시간에 회수.
> - emergency.sh 30줄은 SRE의 runbook 패턴의 미니 버전. Production runbook은 보통 200~1000줄, Confluence/Notion에 문서화. Ch 116 Observability와 함께 문서화 표준 학습.
> - dig vs nslookup vs host — dig가 가장 표준 (BIND 패키지), nslookup은 deprecated 추세, host는 간단한 출력. 두 해 후 Route53 디버깅에서 dig가 표준.
> - lsof는 BSD 기반. Linux의 ss (socket statistics)·netstat이 동등. macOS는 lsof 우선. `lsof -i :8080` 으로 8080 포트 점유 프로세스 확인 — 두 해 후 Flask 개발 시 매일 만남.
> - `top -o cpu`/`top -o mem` 정렬 키 옵션은 macOS BSD 표준. Linux GNU top은 인터랙티브 P/M 키 또는 `-o %CPU`/`-o %MEM`. 자동화 스크립트 호환성 함정.
> - §17 새벽 3시 시나리오의 "load avg > N (=코어 수)" 임계값은 SRE 휴리스틱. 정확한 임계값은 환경마다. Ch 116 Observability에서 정량 임계값 설정 학습.
> - 분량: 이 H6은 약 19,500자(공백 제외). 음독 60~63분. 디벨롭 이력: 2026-04-30 §20 "흔한 실수 응급처치 학습 편" 신설 +1,500자 / 개발자 노트 신설 / 목차 21항목 갱신.
