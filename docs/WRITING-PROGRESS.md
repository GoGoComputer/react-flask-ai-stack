...
## 다음 턴 즉시 할 일
👉 **Ch 005 H5 신규 작성** (데모 — 자경단 5명의 30분 협업 시뮬레이션, 실제 git/gh 명령어 동작 검증)
   - 시나리오: 까미·노랭이가 같은 파일 건드려 conflict 발생 → 5명이 다같이 mergetool로 해결 → PR 생성·리뷰·머지 사이클 한 번 풀로 시연.
   - 실제 동작: git init 임시 demo repo, 가상 5명 commit 시뮬, conflict 만들기, force-with-lease, gh 명령어 시뮬, husky pre-commit hook 발동 시연.
   - 5가지 작은 사고 + 처방 (force-with-lease 거부·rebase abort·stash 분실·CI 빨간불 진단·CODEOWNERS 자동 reviewer).
