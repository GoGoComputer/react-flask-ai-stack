...
## 다음 턴 즉시 할 일
👉 **Ch 005 H7 신규 작성** (원리/내부 — 협업 도구의 GitHub 내부 동작·알고리즘 깊이)
   - branch protection rule이 GitHub 서버에서 어떻게 강제되는가 (push 거부 응답·refs/heads 보호), CODEOWNERS 매칭 알고리즘 (위에서 아래 + 마지막 매치 + 글로브 처리), conflict 알고리즘 (3-way merge·diff3·myers vs histogram·patience), git의 분산 운영 비밀 (refspec·fetch protocol·packfile delta·smart vs dumb HTTP), GitHub Actions 내부 (runner·workflow trigger·secrets 관리), gh CLI 내부 (REST·GraphQL API).
