...
## 다음 턴 즉시 할 일
👉 **Ch 005 H4 신규 작성** (명령어/도구 카탈로그 — 협업 흐름의 25~30개 명령·도구 + 위험도 신호등 + 매일·주간·월간 리듬)
   - PR 흐름 명령어(branch·switch·rebase·push·force-with-lease·pull --rebase 등), GitHub CLI(gh), 리뷰 도구, conflict 도구(diff3·rerere·merge tools), commit 정리(rebase -i·squash·fixup), 위험도 신호등 분류(🟢🟡🔴), 자경단 매일/주간/월간 리듬 한 표.
