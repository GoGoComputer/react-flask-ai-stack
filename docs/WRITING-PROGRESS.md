...
| H7 | 내부동작 | 17,001 | 🟢 | 합격 (.git/ 9슬롯(HEAD·config·description·index·hooks·info·logs·objects·packed-refs·refs)+서가 vs 종이 비유/objects/ 4종 객체(blob·tree·commit·tag) `git cat-file -t/-p` 데모+commit 5필드/SHA-1 content-addressable+`git hash-object`+SHAttered 2017+SHA-256 마이그(--object-format=sha256)/zlib 압축+binary는 LFS/packfile+delta 알고리즘 base 윈도우+`git gc --auto` 6700 loose/50 pack 임계치+push가 빠른 이유/refs/ 41바이트 텍스트+HEAD attached(`ref:`)/detached+packed-refs+refs/remotes/origin/main/index 바이너리+`git ls-files --stage` stage 0/1/2/3+stat 캐싱+손상 응급 `rm .git/index && git reset`/hooks 6종(pre-commit·prepare-commit-msg·commit-msg·pre-push·post-commit·pre/post-receive)+husky+pre-commit framework+자경단 4가지(AWS키·Markdown lint·black·5MB 차단)+hook 5계명/fsck·gc·prune·repack+`count-objects -v`/응급실 한 페이지(reflog·cat-file·branch --contains·blame·log -S pickaxe·diff·fsck·stash·checkout HEAD·rebase --abort·ORIG_HEAD·force-with-lease)+응급실 5계명(한 호흡·--force-with-lease·status 확인·backup-$(date)·동료알림)+reflog 90일+태그 영원/자경단 적용+H8 예고/오해 5+FAQ 5+보너스Q+추신 15) |
## 작성 순서 정책
...
## 다음 턴 즉시 할 일
👉 **Ch 004 H8 신규 작성** (자경단 적용 + 챕터 마무리)
   - 30분 종합 실습(git config 7줄+.gitignore+branch protection+CODEOWNERS+Issue/PR 템플릿+Conventional Commits+pre-commit AWS키 차단+5문서)+다섯 원리 정리+Ch005 예고+우선순위+비용표+오해7+FAQ7+추신.
