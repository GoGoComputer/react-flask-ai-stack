...
## 다음 턴 즉시 할 일
👉 **Ch 006 H7 신규 작성** (원리/내부 — 셸의 진짜 내부 동작)
   - fork-exec 시스템 콜 깊이, process group + session, signal 처리 내부, redirection 내부 (file descriptor 0/1/2 + dup2), pipe 내부 (anonymous pipe + 양방향), background process & job control, 환경변수 inheritance, 셸 기본 명령 vs 외부, login shell vs interactive vs script.
