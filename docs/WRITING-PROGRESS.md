...
## 다음 턴 즉시 할 일
👉 **Ch 006 H6 신규 작성** (운영/스크립트 — bash 스크립트 작성 본격: set -euo pipefail·function·트랩·옵션 파싱·로그·테스트)
   - cleanup.sh 100줄 진화·signal 트랩(EXIT·ERR·INT)·getopts 옵션 파싱·color 로그 함수·shellcheck linter·bats 테스트·stderr/stdout 분리·서브커맨드 구조·자경단 5명 매일 운영 스크립트 5종(deploy·rollback·monitor·migrate·backup).
